...
{    
    ...
    if (total > localStorage.stokMax)
    {
        ...
        $$('#' + content).hide();
        BukaModal(content+ "_notif");
        $$("#"+ content + "_pesan").html("Stok tidak cukup!")

        var obj = document.getElementById(content + "_tutupnotif");
        obj.onclick = function(event) {
            $$("#" + content).show();
            TutupModal(content+ "_notif");
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    app.request({
                        method: 'POST',
                        url: 'http://localhost/myorder/register.php',
                        data: {
                            email : email,
                            password : pass,
                            uname : uname,
    ...
    $$('#btn-login').on('click', function() {
        ...
        else 
        {
            ...
            else
            {
                ...
                //app.request.post('http://localhost/myorder/login.php', 
                app.request.post('http://localhost/myorder/login_crypt.php', 
...
{    
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/riwayat/riwayat.php",
        data: {            
...
{    
    app.request({
        method: "POST",
        url: "http://localhost/myorder/riwayat/listresto.php",
...
function riwayatdetil(id)
{
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/riwayat/riwayatdetil.php",
        data: {
            uid : id
        },
        success: function(data) {
...
function cabangdetil(id)
{
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/cabangdetil.php",
        data: {
            uid : id        
        },
        success: function(data) {
...
function admin_kategoridetil(id)
{
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/admin_kategoridetail.php",
        data: {
            kategori_id : id        
        },
        success: function(data) {
            ...
            var image = "kategori_" + obj[0]['category_id'] + "." + obj[0]['category_img_ext'];
            var image_url = "http://localhost/myorder/images/" + image;

            $$('#kategoridetil_id').html(obj[0]['category_id']);                                               
            document.getElementById("kategoridetil_nama").value = obj[0]['category_name'];

            var img = document.getElementById("kategori_propic");
            img.setAttribute('src', image_url);
            img.setAttribute('onerror', 'this.onerror=null;this.src="./img/drawable-ldpi/Logo.png";');
        }
    });
}
...
function admin_menu()
{
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/menulist.php",
        data: {  
            storeid : id
        },
        success: function(data) {
                ...
                {   
                    ...
                    if(obj['data'][i]['menu_discount'] > 0)
                    {
                        ...
                        $$('#admin_menulist').append(
                            "<a href='/menudetil/"+obj['data'][i]['menu_id']+"' class='card'>"+
                            "<img src='"+image_url +"'"+"onerror='this.onerror=null;this.src="+'"./img/drawable-ldpi/Logo.png";' + "'>"+
                                "<div class='menu-info'>"+
                                    "<h5>"+obj['data'][i]['menu_name']+"</h5>"+
                                    "<p>"+obj['data'][i]['menu_desc']+"</p>"+
                                    "<h5 style='color: #707070;  text-decoration: line-through' class='price'>Rp. "+formatRupiah(obj['data'][i]['menu_sell_price']) +"</h5>"+
                                    "<h5 class='price'>&nbsp; Rp. "+formatRupiah(total)+"</h5>"+
                                "</div>"+
                            "</a>"
                        );
                    }
                        ...
                        $$('#admin_menulist').append(
                            ...
                            "<img src='"+image_url +"'"+"onerror='this.onerror=null;this.src="+'"./img/drawable-ldpi/Logo.png";' + "'>"+
                                "<div class='menu-info'>"+
                                    "<h5>"+obj['data'][i]['menu_name']+"</h5>"+
                                    "<p>"+obj['data'][i]['menu_desc']+"</p>"+
                                    "<h5 class='price'>Rp. "+formatRupiah(obj['data'][i]['menu_sell_price']) +"</h5>"+
                                "</div>"+
                            "</a>"
                        );
                    }                 
...
{    
    app.request({
        ...
        success: function(data) {
            ...
            var image = "menu_" + obj['data'][0]['menu_id'] + "." + obj['data'][0]['menu_img_ext'];
            var image_url = "http://localhost/myorder/images/" + image;
            
            $$('#menudetil_menuid').html(obj['data'][0]['menu_id']);
            ...
            document.getElementById("menudetil_stok").value = obj['data'][0]['menu_stok'];
            document.getElementById("menudetil_menudiskon").value = obj['data'][0]['menu_discount'];
            document.getElementById("menudetil_menudesc").value = obj['data'][0]['menu_description'];

            var img = document.getElementById("menu_propic");
            img.setAttribute('src', image_url);
            img.setAttribute('onerror', 'this.onerror=null;this.src="./img/drawable-ldpi/Logo.png";');
...
function admin_karyawan()
{
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/karyawan.php",
        data: {  
            storeid : id
        },
        success: function(data) {            
...
{    
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/karyawandetil.php",
        data: {  
            uid : id,            
        },
        success: function(data) {
            ...
            //document.getElementById("karyawandetil_password").value = localStorage.password;
...
{    
    app.request({
        method: "POST",
        url: "http://localhost/myorder/pesanan/jenispembayaran.php",        
        success: function(data) {
            var obj = JSON.parse(data);                                    
            if(obj['result'] == "Berhasil")
            {
                ...
                for(var i =0; i < obj['data'].length; i++)
                {
                    var image = "jenispembayaran_" + obj['data'][i]['payment_type_id'] + "." + obj['data'][i]['payment_img_ext'];
                    var image_url = "http://localhost/myorder/images/" + image;
...
{    
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/jenispembayarandetil.php",  
        data: {  
            uid : id,            
        },
        success: function(data) {
...
{    
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/laporan.php",
        data: {            
            storeid : store,
...
function admin_laporan_menulist()
{
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/laporan_menulist.php",
        data: {            
            storeid : store,
            tanggalawal : tanggalstart,
            tanggalakhir : tanggalend,            
...
{    
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/keranjang/keranjang_menulist.php",
        data: {
...
function biaya_antar()
{
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/pesanan/biaya_antar.php",
        data: {
...
function pesan() {
    ...
    else
    {
        ...
        else
        {
            app.request({
                method: "POST",
                url: "http://localhost/myorder/keranjang/pesan.php",
                data: {
                    role : localStorage.user_role,
                    uid : uid,   
                    nama : customer_name,
...
function update_stok(id_menu, jumlah)
{
    app.request({
        method: "POST",
        url: "http://localhost/myorder/keranjang/update_stok.php",
        data: {
            menu_id : id_menu,
            jumlah : jumlah
        },
...
{    
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/stok/ubahstok.php",
        data: { 
            user_id : id,
            stokbaru : jumlah
        },
...
function listbanner(halaman)
{
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/bannerlist.php",
        data: {
            store_id : store_id
        },
        success: function(data) {                        
            ...
            if (halaman == "beranda")
            {
                if (obj['status'] == true)
                {
                    ...
                    for(var i =0; i < obj['data'].length; i++)
                    {                 
                        var image =  "banner_" + obj['data'][i]['banner_id'] + "." + obj['data'][i]['banner_img_ext'];                        
                        var image_url = "http://localhost/myorder/images/" + image;
...
function bannerdetil(id)
{
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/bannerdetil.php",
        data: {
            uid:id,
        },
        success: function(data) {       
            ...
            console.log(obj['data'][0]['menu_aim_id'])
            
            //BUAT NGECHECK EMPTY STRING
            if(obj['data'][0]['menu_aim_id'] === null || obj['data'][0]['menu_aim_id'] === "")
            {
                document.getElementById('radio_kategori').checked = true;
                admin_kategori_combobox(obj['data'][0]['category_aim_id']);
            }
            else if(obj['data'][0]['category_aim_id'] === null || obj['data'][0]['category_aim_id'] === "")
            {
...
function simpanGambar(halaman, id, nama)
{
    ...
    if (path == "./img/drawable-ldpi/Logo.png")
    {
        ext = path.split(".")[2];
    }
    else
    ...
    ft.upload(imgUri,
    encodeURI("http://localhost/myorder/uploadpic.php"),
    function(result){
       // app.dialog.alert("Simpan Gambar Berhasil","Success");        
    }, 
    function(error){ 
...
function admin_search(halaman)
{
    ...
    app.request({
        method: "POST",
        url: "http://localhost/myorder/admin/admin_search.php",
        data: {
            halaman:halaman,
            nama:nama,
            //menu
        ...
        success: function(data) {              
            ...
            if (halaman == 'menu')
            {
                ...
                for(var i =0; i < obj.length ; i++)
                {      
                    var image = "menu_" + obj[i]['menu_id'] + "." + obj[i]['menu_img_ext'];
                    var image_url = "http://localhost/myorder/images/" + image;

                    if (obj[i]['menu_discount'] > 0)
                    {
                        var discount = (obj[i]['menu_sell_price'] * obj[i]['menu_discount'] / 100);
                        var total = obj[i]['menu_sell_price'] - discount;
    
                        $$('#admin_menulist').append(
                            "<a href='/menudetil/"+obj[i]['menu_id']+"' class='card'>"+
                            "<img src='"+image_url +"'"+"onerror='this.onerror=null;this.src="+'"./img/drawable-ldpi/Logo.png";' + "'>"+
                                "<div class='menu-info'>"+
                                    "<h5>"+obj[i]['menu_name']+"</h5>"+
                                    "<p>"+obj[i]['menu_description']+"</p>"+
                                    "<h5 style='color: #707070;  text-decoration: line-through' class='price'>Rp. "+formatRupiah(obj[i]['menu_sell_price']) +"</h5>"+
                                    "<h5 class='price'>&nbsp; Rp. "+formatRupiah(total)+"</h5>"+
                                "</div>"+
                            "</a>"
                        );
                        ...
                        $$('#admin_menulist').append(
                            ...
                            "<img src='"+image_url +"'"+"onerror='this.onerror=null;this.src="+'"./img/drawable-ldpi/Logo.png";' + "'>"+
                                "<div class='menu-info'>"+
                                    "<h5>"+obj[i]['menu_name']+"</h5>"+
                                    "<p>"+obj[i]['menu_description']+"</p>"+
                                    "<h5 class='price'>Rp. "+formatRupiah(obj[i]['menu_sell_price']) +"</h5>"+
                                "</div>"+
                            "</a>"
                        );
                    }  
            ...
            else if (halaman == "kategori")
            {
                ...
                for(var i =0; i < obj['data'].length ; i++)
                {
                    var image = "kategori_" + obj['data'][i]['category_id'] + "." + obj['data'][i]['category_img_ext'];
                    var image_url = "http://localhost/myorder/images/" + image;
                    $$('#listkategori').append(
                        "<li style='padding: 8px 0px; border-bottom: 1px solid rgba(0,0,0,0.1);'>" +                
...
function count_pesananBaru()
{
    app.request({
        method: "POST",
        url: "http://localhost/myorder/beranda/count_pesananBaru.php",
        data: {            
            store_id: localStorage.cabang,
        },
        success: function(data) {
...
function onBackKeyDown() {
    ...
    else
    {
        ...
        {   
            var currentPath = app.views.main.router.currentRoute.path;
            var status = currentPath.split('/');

            if (app.views.main.router.currentRoute.path == '/beranda/')
            {
                navigator.appexitApp();
            }
            else
            {
                if(status[1] == "menudetil")
                {
                    app.views.main.router.navigate('/menu/');
                }
                else if(status[1] == "bannerdetil")
                {
                    app.views.main.router.navigate("/banner/");
                }
                else if(status[1] == "karyawanedit")
                {
                    app.views.main.router.navigate("/karyawan/");
                }
                else if(status[1] == "pembayarandetil")
                {
                    app.views.main.router.navigate("/pembayaran/");
                }
                else if(status[1] == "cabangdetil")
                {
                    app.views.main.router.navigate("/cabang/");
                }
                else if(status[1] == "kategoriedit")
                {
                    app.views.main.router.navigate("/kategori/kategoriedit")
                }
                else if(status[1] == "menu" || status[1] == "banner" || status[1] == "karyawan" || 
                        status[1] == "pembayaran" || status[1] == "cabang" || status[1] == "kategori")
                {
            ...
            //! menghilangkan bbrp css display:none
            // if(app.views.main.router.currentRoute.path == "/beranda/")
            // {
            //     navigator.app.exitApp();
            // }
