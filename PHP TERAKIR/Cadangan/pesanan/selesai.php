<?php
  ...
  $sql = "UPDATE transaction SET transaction_status='Selesai', payment_id = ?, transaction_total_paid = ?, transaction_total_change = ?  WHERE transaction_id = ?";


  $stmt = $mysqli->prepare($sql);
  $stmt->bind_param('iiii', $payment_id, $total_pembayaran, $total_kembalian ,$transaction_id);
  $stmt->execute();
